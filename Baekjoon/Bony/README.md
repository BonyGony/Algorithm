...
- 유형
  - 구현
- 풀이
  - 각 문장을 띄어쓰기 기준으로 list로 만들어 준다.
  - 각 단어를 slicing 기법을 통해 반전 시켜 새로운 배열에 넣는다.
  - join을 통해 반전된 단어들을 띄어쓰기 하여 출력한다.
...
- 유형
  - 구현
- 풀이
  - 요일과 비교해서 같으면 total을 증가시키고, total 출력
...
- 유형
  - 구현
- 풀이
  - a, b, c를 계산한 값을 str으로 만들어 list로 만들어 준다
  - 0부터 9까지 0으로 초기화한 Dic에 list의 원소에 해당하는 값을 1씩 증가시킨다.
  - Dic의 value값을 전부 출력해 준다.
...
- 유형
  - 구현
- 풀이
  - 0이 나올때마다 t값을 1 증가시킨다.
  - t 값에 따라 도, 개, 걸, 윷, 모를 찾아준다.
